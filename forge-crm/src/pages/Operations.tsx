...
export default function Operations() {
  ...
  }, [filteredEngagements, page])

  const handleExport = () => {
    const headers = ['Customer', 'Type', 'Consultant', 'Hours Used', 'Hours Budget', 'Status', 'Revenue']
    const rows = filteredEngagements.map(e => [
      e.organizationName, e.type, e.consultant,
      e.hoursUsed, e.hoursBudget, e.status, e.revenue
    ])
    ...
    a.href = url
    a.download = 'engagements-report.csv'
    a.click()
  ...
  return (
    <div className="space-y-6">
      ...
      <Card
        ...
        noPadding
        action={
          <button onClick={handleExport} className="px-3.5 py-1.5 rounded-lg border border-forge-border text-sm font-medium text-forge-text hover:bg-forge-bg transition-colors">
            Export Report
          </button>
        }
      >
        ...
        <Pagination total={filteredEngagements.length} page={page} perPage={perPage} onPageChange={setPage} />
      </Card>
    </div>
  )
}
